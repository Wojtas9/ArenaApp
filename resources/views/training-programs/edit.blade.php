...
@section('content')
        ...
        <div class="flex flex-col md:flex-row max-w-[1400px] mx-auto gap-6 relative z-20">
            ...
            @include('layouts.partials.sidebar', [
                ...
                'navLinks' => [
                    [
                        ...
                        'active_check_route_name' => 'admin.users',
                    ],
                    [
                        'icon' => '🏟️',
            ...
            <div class="flex-1 p-8 bg-white rounded-4xl shadow-lg drop-shadow-xl/50 min-h-[500px]">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Training Program</h1>
                    <div class="flex gap-2">
                        <a href="{{ route('training-programs.show', $trainingProgram) }}"
                ...
                <form method="POST" action="{{ route('training-programs.update', $trainingProgram) }}" id="program-form">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" id="status"
                            ...
                            required>
                            <option value="active"
                                {{ old('status', $trainingProgram->status) == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="completed"
                                {{ old('status', $trainingProgram->status) == 'completed' ? 'selected' : '' }}>Completed
                            </option>
                            <option value="cancelled"
                                {{ old('status', $trainingProgram->status) == 'cancelled' ? 'selected' : '' }}>Cancelled
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div id="editor-container" class="bg-white rounded-lg border border-gray-300 mb-2"
                            style="min-height: 200px; max-height: none;"></div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="bg-white rounded-lg border border-gray-300 p-4">
                            ...
                            <div class="flex">
                                ...
                                <button type="button" id="add-todo-btn"
                                    class="bg-[#8C508F] hover:bg-[#734072] text-white px-4 py-2 rounded-r-lg">Add</button>
                            </div>

                            <!-- Hidden input to store the JSON todo list -->
                            <input type="hidden" id="todo_list" name="todo_list"
                                value="{{ old('todo_list', json_encode($trainingProgram->todo_list ?? [])) }}">
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set initial content
            const initialContent = {!! json_encode($trainingProgram->description) !!};
            if (initialContent) {
