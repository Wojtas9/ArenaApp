<h1>Create Diet Plan</h1>
    ...
    <form action="{{ route('diet.diet-plans.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-4">
            ...
            <select name="nutritional_goal_id" id="nutritional_goal_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                ...
                @foreach($nutritionalGoals as $goal)
                    <option value="{{ $goal->id }}" {{ old('nutritional_goal_id') == $goal->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline h-32 resize-none">{{ old('description') }}</textarea>
            @error('description')
                <div class="text-red-500 text-xs italic mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <button type="submit" class="bg-[#8C508F] hover:bg-[#7a457d] text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Create Plan</button>
    </form>